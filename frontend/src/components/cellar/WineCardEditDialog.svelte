  ...
  <Dialog.Content class="sm:max-w-[425px]">
    ...
    <div class="grid gap-4 py-4 justify-start items-start">
      ...
      <div class="grid grid-cols-4 items-center gap-4">
        ...
        <Input type="text" id="vintage" name="vintage" value={wine.vintage} class="text-sm col-span-3" />
      </div>
      <div class="grid grid-cols-4 items-center gap-4">
        <Label for="format" class="text-sm text-muted-foreground text-right">Format</Label>
        <Input type="text" id="format" name="format" value={wine.format} class="text-sm col-span-3" />
      </div>
      <div class="grid grid-cols-4 items-center gap-4">
        <Label for="price" class="text-sm text-muted-foreground text-right">Price</Label>
        <Input type="number" id="price" name="price" value={wine.summary.price} class="text-sm col-span-3" />
      </div>
      <div class="grid grid-cols-4 items-center gap-4">
        <Label for="date" class="text-sm text-muted-foreground text-right">Date</Label>
        <Input type="date" id="date" name="date" value={new Date().toISOString().slice(0, 10)} class="text-sm col-span-3" />
      </div>
    </div>
    <Dialog.Footer>
